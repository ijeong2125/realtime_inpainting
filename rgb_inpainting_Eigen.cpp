...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    std::vector<cv::Mat> bgr(3), output(3);
    cv::split(img,bgr);
    Eigen::MatrixXd X_B,X_G,X_R;
    cv::cv2eigen(bgr[0],X_B);
    cv::cv2eigen(bgr[1],X_G);
    cv::cv2eigen(bgr[2],X_R);
    X_B = X_B/255.;
    X_G = X_G/255.;
    X_R = X_R/255.;

    int xStart,xEnd,yStart,yEnd;
    useBoundary(X_B, xStart, xEnd, yStart, yEnd);
    
    if(xStart==-1)
    {
        sensor_msgs::Image::Ptr img_msg = cv_bridge::CvImage(msg->header,"bgr8",img).toImageMsg();
    ...
    {
        Eigen::MatrixXd X;
        X = X_B.block(yStart,xStart,yEnd-yStart,xEnd-xStart);
        int m = X.rows();
        int n = X.cols();
        int mn = m*n;
        Eigen::MatrixXd B;
        ...
        for(int color=0;color<3;color++)
        {
            ...
            switch(color)
            {
                case 0:
                    ...
                    break;
                case 1:
                    X = X_G.block(yStart,xStart,yEnd-yStart,xEnd-xStart);
                    X_origin = X_G;
                    break;
                case 2:
                    X = X_R.block(yStart,xStart,yEnd-yStart,xEnd-xStart);
                    X_origin = X_R;
                    break;
            }
            ...
            rho = Eigen::VectorXd::Ones(m*(n-1)+(m-1)*n)/50;
            int iter = 15;
            ...
            Eigen::MatrixXd X_recon(m,n);
            X_recon = Eigen::Map<Eigen::MatrixXd>(X_recon_vec.data(),m,n);
            X_origin.block(yStart,xStart,yEnd-yStart,xEnd-xStart) = X_recon;
            cv::eigen2cv(X_origin,output[color]);
        }

        cv::Mat output_bgr,output_int;
        cv::merge(output,output_bgr);
        output_bgr *= (int)255;
        output_bgr.convertTo(output_int,CV_8UC3);

        sensor_msgs::Image::Ptr img_msg = cv_bridge::CvImage(msg->header,"bgr8",output_int).toImageMsg();
        img_pub.publish(img_msg);
